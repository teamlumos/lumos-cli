"""Request command group for Lumos CLI.

Commands:
    lumos request create - Request access to an app
    lumos request list   - List access requests
    lumos request status - Check the status of a request
    lumos request poll   - Poll a request for completion
    lumos request cancel - Cancel a pending request
"""
...
from click_extra import confirm, echo, group, option, prompt
...
@request.command("create", help="Request access to an app")
...
def create(
    ...
    command_str = f'lumos request create --app {selected_app.id} {permission_flags} --reason "{reason}"'
    if duration:
...
@request.command("list", help="List access requests")
@option(
    "--for-user",
    default=None,
    type=str,
